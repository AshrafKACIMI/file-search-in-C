...
void search(const char * path, const char * nom_fichier, options opt, int nb_niveaux){
// Rôle : traverse le répertoire et, éventuellement, ses sous-répertoires à la recherche du fichier de nom nom_fichier, avec des options
        ...
        char filename [255];          // sert de path absolu pour explorer les fichiers du répertoire
        DIR *dir = NULL;              //simule un répertoire
        
        // ----------- phase de recherche -------- //
        if ( (dir = opendir(path)) == NULL){
            printf("\nLe chemin que vous avez spécifié n'existe pas\n");
            return;   
        }
        ...
        while ((entry = readdir(dir)) != NULL) {               //readdir(*DIR) retourne un 'dirent' s'il y a encore des fichiers à lire, NULL sinon
            ...
            /* cas d'un fichier */
                    ...
                    if (!strcmp(entry->d_name, nom_fichier )){ // si nom_fichier == le fichier courant
...
void main(int argc, char** argv){
    trouve = 0;
    int i = 0, 
        profondeur = (int)INFINITY;
    options opt;
    char* s = getwd(NULL);
    init_options(&opt);
    for (i = 2; i < argc; i++)
        get_option(argv[i], &opt, &profondeur);
    search(argv[1], argv[argc-1], opt, profondeur);  //respectivement : path nom_fichier options
    if (!trouve) 
